...
| `shap` | 模型可解释性 | 特征重要性、SHAP 值解释、博弈论归因 |
| `exploratory-data-analysis` | 数据探索 | 数据画像、相关性分析、异常检测、分布分析 |
| `statistical-analysis` | 统计分析 | 假设检验、置信区间、描述统计、推断统计 |
| `matplotlib` | 基础可视化 | 线图、散点图、柱状图、自定义图表 |
| `seaborn` | 高级可视化 | 统计图表、热力图、分布图、回归图 |
| `dask` | 分布式计算 | 大数据并行处理、分布式 DataFrame、任务调度 |
